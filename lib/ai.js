...
function ai_init(imgd, LX, LY, types){
...
function ai_draw(x,y, time){
    ...
    //        drawcircle(ai_paths[i][j].pos.x, ai_paths[i][j].pos.y, 1);
    //}
...
function ai_update(x, y, vx, vy, time, lx, ly){
    for (var i=0; i<ai_monsters.length; i++){
        ...
        if (ai_stuck[i]){
            if (time - ai_last_move[i] > 10)
                ai_stuck[i] = false;

            vx[ai_monsters[i]] = SPEED * (2*Math.random() - 1);
            vy[ai_monsters[i]] = SPEED * (2*Math.random() - 1);
        }
        else {
            if (gdist < 50){
                ...
                if (time > ai_next_time[i]){
                    ai_paths[i] = ai_find_path([mx, my], [x[0], y[0]], ai_waypoints.nodes, lx, ly);
                    ai_next_time[i] = time + gdist / SPEED / 4;
                }

                if ((time - ai_last_move[i]) > 1){
                    ai_stuck[i] = true;
                }
            }
        }
